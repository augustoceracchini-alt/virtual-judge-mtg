...
- `npm run sonda-coerenza [ripetizioni] [libero]` — misura la COERENZA: la stessa domanda ripetuta N volte, e quante versioni diverse tornano. Va lanciata **due volte**, `npm run sonda-coerenza` e `npm run sonda-coerenza -- 5 libero` (quest'ultima disattiva la generazione deterministica e mostra com'era prima), e confrontata. Misura separatamente la FASE A (l'amplificatore) e la FASE D, e per quest'ultima riporta tre numeri distinti: varianti di testo, di regole citate e di conclusione — **solo l'ultimo conta per l'utente**, e può benissimo essere 1 mentre il primo è 5. Usa `gemini-3.5-flash-lite` e fa 2 × N richieste. **Il piano gratuito consente 15 richieste al MINUTO per modello**, non solo un tetto giornaliero (misurato dal vivo: l'errore 429 riporta `GenerateRequestsPerMinutePerProjectPerModel-FreeTier, limit: 15`), quindi la sonda mette 4,5 secondi di pausa fra una richiesta e l'altra e riprova una volta se il tetto scatta lo stesso. Senza quella pausa una passata da 10 ripetizioni moriva a metà strada buttando via anche le misure già raccolte
...
- **La 113.7a: la diagnosi che il progetto portava avanti era SBAGLIATA, misurata e corretta il 24 agosto 2026.** Per tre osservazioni si era scritto qui che il giudice «parafrasa invece di citare» una regola che ha davanti, e che il posto per intervenire fosse il prompt della FASE D. Due misure lo smentiscono. **Prima**: ripetendo 5 volte la stessa domanda sulla sola FASE D, con la 113.7a fra gli estratti, il modello la cita **5 volte su 5** — quindi quando ce l'ha, la cita. **Seconda**: nel turno reale del 24 agosto la 113.7a **non era affatto fra gli estratti** (capitoli recuperati: 714, 305, 608, 303, 301, 205, 110, 702 — nessun 113), e il giudice ha citato la 608.2h, cioè la fonte migliore che aveva. Non era un difetto di citazione: era un difetto di **recupero**, e si affronta in `lib/rules.ts`. Resta valida la sola osservazione più vecchia, in cui la regola era effettivamente presente e non citata. Non è una lacuna di recupero ma variabilità del modello nel citare la fonte che ha davanti; se ricapita, il posto in cui intervenire è il prompt della FASE D, non `lib/rules.ts`. Non si era ripetuto il 19 agosto 2026, ma **si è ripetuto il 24 agosto 2026**: al terzo turno il giudice ha di nuovo parafrasato («un'abilità che si trova sulla pila è un oggetto indipendente dalla fonte che l'ha generata») senza il numero, con la 113.7a presente negli estratti — verificato cercandola nei log. Il caso del 24 agosto 2026 **non è però una ripetizione del difetto**, come si era scritto di primo acchito: quel turno non aveva affatto la 113.7a fra gli estratti (nessun capitolo 113 fra i recuperati), quindi il giudice ha usato la 608.2h, la fonte migliore che aveva. Vedi la voce corretta in "Cosa manca ancora": il problema è di recupero, non di citazione
